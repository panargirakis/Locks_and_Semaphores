...
void rwlock_init(rwlock_t *rw, unsigned int numTeams) {
    ...
    sem_init(&rw->lockServiceQue, 0, numTeams/2 > 0 ? numTeams/2 : 1);
//    sem_init(&rw->lockServiceQue, 0, 1);
    sem_init(&rw->lockTeams, 0, numTeams);
}

int rwlock_acquire_pirLock(rwlock_t *rw) {
...
//    printf("acq. pirlock: success get pirlock\n");
    ...
    int team;
    sem_getvalue(&rw->lockTeams, &team);
    return 4 - team; // team 3 -> team 1 and team 0 -> team 4
}
...
int rwlock_acquire_ninLock(rwlock_t *rw) {
    sem_wait(&rw->lockServiceQue); // wait in line to be serviced
